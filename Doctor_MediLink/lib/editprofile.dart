...
class _EditProfilePageState extends State<EditprofilePage> {
  ...
  Future<void> _uploadImage(File image) async {
    try {
      final fileName = 'doctorphoto-${DateTime.now().millisecondsSinceEpoch}';
      ...
      await supabase.from('tbl_doctor').update({'doctor_photo': imageUrl}).eq(
          'doctor_id', supabase.auth.currentUser!.id);
    } catch (e) {
  ...
  Future<void> fetchUser() async {
    try {
      ...
      setState(() {
        user = response;
        nameController.text = user!['doctor_name'] ?? '';
        emailController.text = user!['doctor_email'] ?? '';
        contactController.text = user!['doctor_contact'] ?? '';
        dobController.text = user!['doctor_dob'] ?? '';
        image = user!['doctor_photo'];
        isLoading = false;
      });
    } catch (e) {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await supabase.from('tbl_doctor').update({
          ...
          'doctor_contact': contactController.text,
          'doctor_dob': dobController.text,
        }).eq('doctor_id', uid);
